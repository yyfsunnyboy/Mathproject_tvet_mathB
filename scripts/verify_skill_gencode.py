...
def main() -> None:
    ...
    verified = [v for v in reg.get("verified_problem_types", []) if isinstance(v, dict) and v.get("skill_id") == TARGET_SKILL]
    first_error = ""
    ...
    if ok_registry:
        try:
            mod = importlib.import_module("skills.vh_數學B1_AbsoluteValue")
            for _ in range(args.sample_count):
                q = mod.generate(level=1)
                samples.append(q)
                unique_pts.add(str(q.get("problem_type_id", "")))
    ...
    test_cmd = [sys.executable, "-m", "pytest", "tests/test_b1_absolute_value_skill_wrapper.py", "-q"]
    ...
    passed = ok_registry and ok_generate and ok_pytest
    ...
    lines += ["", "## Verified Entries", "```json", json.dumps(verified, ensure_ascii=False, indent=2), "```", "", "## Pytest Output", "```text", (tr.stdout + tr.stderr).strip(), "```", "", "## Samples", "```json", json.dumps(samples, ensure_ascii=False, indent=2), "```"]
    report.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"success": passed, "report": str(report), "first_error": first_error}, ensure_ascii=False))
