# -*- coding: utf-8 -*-
"""Package and publish the sealed Math B2 1-3 V3 components without DB writes."""
...
def _load_generate(example_id: int, skill_id: str) -> Any:
    ...
    module_spec = importlib.util.spec_from_file_location(f"b2_13_phase3_{example_id}", path)
...
def _tracker_snapshot() -> sqlite3.Connection:
    ...
    for example_id, component in sorted(SPECS.items()):
        ...
        if payload.get("presentation_mode") == "single_choice":
            spec.update({"choices": payload.get("choices"), "answer": payload.get("correct_answer")})
        conn.execute(
...
def _atomic_copy(source: Path, destination: Path) -> None:
    ...
    temporary = destination.with_name(f".{destination.name}.b2_13_phase3.tmp")
...
def package_and_publish() -> dict[str, Any]:
    ...
    staging_root = Path(tempfile.mkdtemp(prefix="b2_13_phase3_", dir=staging_parent))
    ...
    try:
        for skill_id, expected_count in SKILL_COMPONENTS.items():
            ...
            synced = _sync_staging_v3_component_sources(staging_root, skill_id, project_path=PROJECT_ROOT)
            if compiled["component_count"] != expected_count:
                raise RuntimeError(f"wrapper_count_mismatch:{skill_id}")
            if synced["component_count"] != expected_count:
                raise RuntimeError(f"component_sync_count_mismatch:{skill_id}")
            run_v3_smoke(staging_root, skill_id)
            results[skill_id] = {"wrapper_components": expected_count, "published": 0, "staging_smoke": "passed", "production_smoke": "pending"}
        ...
        for skill_id, expected_count in SKILL_COMPONENTS.items():
            ...
            _atomic_copy(stage_skill / "component_manifest.json", production_skill / "component_manifest.json")
            _atomic_copy(staging_root / "skills" / f"{skill_id}.py", PROJECT_ROOT / "skills" / f"{skill_id}.py")
            run_v3_smoke(PROJECT_ROOT, skill_id)
            results[skill_id]["published"] = expected_count
            results[skill_id]["production_smoke"] = "passed"
    finally:
        conn.close()
    return {"status": "published", "staging_root": str(staging_root), "skills": results, "component_total": sum(SKILL_COMPONENTS.values()), "production_db_written": False}
