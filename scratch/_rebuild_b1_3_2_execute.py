# -*- coding: utf-8 -*-
"""B1 3-2 FULL rebuild: per-example Phase2 → 20-seed → package → publish → smoke.
...
DB_TEXTS = {
    ...
    for r in _db_tmp.execute(
        "SELECT id, problem_text FROM textbook_examples WHERE source_volume LIKE '%B1%' AND source_section LIKE '3-2%'"
    ).fetchall()
}
_db_tmp.close()
...
def _inject_source_text(comp_dir: Path, problem_text: str) -> None:
    ...
    if "SOURCE_PROBLEM_TEXT" not in gen:
        ...
        if idx >= 0:
            ...
            insert_at = end + 1
            gen = gen[:insert_at] + f"\nSOURCE_PROBLEM_TEXT = {literal}\n" + gen[insert_at:]
        else:
            gen = f"SOURCE_PROBLEM_TEXT = {literal}\n" + gen
    else:
        gen = re.sub(
            r"^SOURCE_PROBLEM_TEXT = .*",
            f"SOURCE_PROBLEM_TEXT = {literal}",
            gen,
            count=1,
            flags=re.M,
        )
...
def _patch_answer_contract_files(comp_dir: Path, sample: dict[str, Any], *, force_choice: bool) -> str:
    ...
    if topology == "choice" and "3-2 force single_choice" not in gen:
        inject = '''
...
def main() -> int:
    ...
    skills = ["vh_數學B1_RemainderTheorem", "vh_數學B1_FactorTheorem"]
    publish_results = []
    for skill in skills:
    ...
    out = ROOT / "scratch" / "_b1_3_2_rebuild_results.json"
