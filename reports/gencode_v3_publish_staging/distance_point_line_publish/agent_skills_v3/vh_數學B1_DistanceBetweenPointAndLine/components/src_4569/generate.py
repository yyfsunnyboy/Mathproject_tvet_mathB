...
TEXTBOOK_EXAMPLE_ID = 4569


def generate(level: int = 1, seed: int | None = None, **kwargs: Any) -> dict[str, Any]:
    matrix = build_line_equation_matrix(
        seed=seed,
        line_type="distance_from_point_to_line_parameter",
        curriculum_profile="vocational_high_b",
        difficulty_profile="easy",
        constraints={'solution_cardinality': 'solution_set'},
    )
    ...
    payload = convert_line_equation_matrix_to_question_payload(
        ...
        domain_operation="distance_from_point_to_line_parameter",
    )
    if component_id:
