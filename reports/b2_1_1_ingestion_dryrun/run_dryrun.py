...
if not captured:
 # Supplementary existing Phase1-3 dry-run after recording the unmodified V3 gate failure.
 from core.textbook_importer_v3_phase3_dryrun import build_b2_11_curriculum_info
 from models import SkillCurriculum
 with app.app_context():
  info=build_b2_11_curriculum_info(report['latex_docx'],pair.original_docx.name)
  outline=SkillCurriculum.query.filter_by(skill_id='outline_vocational_數學B2_11').first()
  info['chapter']=outline.chapter
  info['section']=outline.section
  lines=tp.phase1_extract_docx_lines(report['latex_docx'])
  save('phase1_lines.json',lines)
  blocks=tp.phase2_deterministic_block_slice(lines,source_scope=info['source_scope'],curriculum_info=info)
  save('phase2_meta.json',tp._DOCX_BLOCK_META)
  parsed=tp.phase3_ai_metadata_alignment(sorted(blocks),info,queue.Queue())
  capture(parsed,dict(tp._DOCX_BLOCK_META),info)
if captured:
