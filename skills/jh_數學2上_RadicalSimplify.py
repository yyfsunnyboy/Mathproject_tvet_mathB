# -*- coding: utf-8 -*-
# Pattern skill: p0_simplify — 單一根式化簡
...
def check(user_answer, correct_answer):
    ...
    if "," in c:
        ...
        if len(parts) == 2:
            try:
                coeff, rad = float(parts[0]), int(parts[1])
                if rad == 1:
                    return {"correct": False, "result": f"答案錯誤。正確答案為：{coeff}"}
                return {"correct": False, "result": f"答案錯誤。正確答案為：{c}"}
            except (ValueError, TypeError):
                pass
