...
def generate(level=1):
    ...
    question_text = f"請計算：$\\frac{{{f1.numerator}}}{{{f1.denominator}}} \\div \\frac{{{f2.numerator}}}{{{f2.denominator}}}$"
    correct_answer = f"{result.numerator}/{result.denominator}" if result.denominator != 1 else str(result.numerator)

    context_string = f"計算分數除法，等於乘以除數的倒數。"
