...
def generate(level=1):
    ...
    生成一道「向量正射影」的題目。
    """
    a = np.array([random.randint(-5, 5) for _ in range(3)])
    b = np.array([random.randint(-3, 3) for _ in range(3)])
    while np.all(b==0): b = np.array([random.randint(-3, 3) for _ in range(3)])
