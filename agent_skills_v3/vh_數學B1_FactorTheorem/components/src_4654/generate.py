...
SOURCE_PROBLEM_TEXT = "已知兩多項式$f\\left( x \\right)$與$g\\left( x \\right)$除以2x − 1的餘式分別為1與−1，試求多項式${{\\left[ f\\left( x \\right) \\right]}^{2020}}+{{\\left[ g\\left( x \\right) \\right]}^{2020}}$除以$x-\\frac{1}{2}$的餘式。"
