...
def generate(level: int = 1, seed: int | None = None, **kwargs: Any) -> dict[str, Any]:
    rng = random.Random(seed if seed is not None else 3853)
    
    # 1. Randomize parameters while maintaining original textbook spirit (mean=10, std=8)
    if seed is None:
        mean = 10
        std = 8
        k1 = 3
        ...
        mean = rng.choice([5, 10, 15, 20, 25, 30, 35, 40])
