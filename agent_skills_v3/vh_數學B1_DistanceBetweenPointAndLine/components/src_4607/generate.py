...
PROBLEM_TYPE_ID = "distance_from_point_to_line_parameter_single_choice_scalar"
TEXTBOOK_EXAMPLE_ID = 4607


def generate(level: int = 1, seed: int | None = None, **kwargs: Any) -> dict[str, Any]:
    matrix = build_line_equation_matrix(
        seed=seed,
        line_type="distance_from_point_to_line_parameter_single_choice_scalar",
        curriculum_profile="vocational_high_b",
        difficulty_profile="hard",
        constraints={'solution_cardinality': 'single', 'choice_value_shape': 'scalar'},
    )
    ...
    payload = convert_line_equation_matrix_to_question_payload(
        ...
        domain_operation="distance_from_point_to_line_parameter_single_choice_scalar",
    )
    if component_id:
