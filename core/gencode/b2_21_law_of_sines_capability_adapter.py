# -*- coding: utf-8 -*-
"""Payload contracts for B2 §2-1 Law of Sines shared capabilities."""
...
_FIXED = "trigonometry.law_of_sines"
...
OPERATION_PAYLOAD_CONTRACTS["solve_side_and_circumradius_by_sines"].update(
    {"checker": "multi_part_answer_checker"}
)
OPERATION_PAYLOAD_CONTRACTS["solve_side_ratio_by_law_of_sines"].update(
    {"checker": "single_choice_checker"}
)
OPERATION_PAYLOAD_CONTRACTS["solve_side_by_law_of_sines"].update(
    {"checker": "single_choice_checker"}
)


def adapt_b2_21_law_of_sines_matrix(
    matrix: dict[str, Any],
    *,
    ...
    **kwargs: Any,
    ...
    if contract is None:
        raise ValueError(f"unsupported_b2_21_law_of_sines_operation:{domain_operation}")
    if not validate_trigonometry_law_of_sines_matrix(matrix):
        raise ValueError(f"invalid_b2_21_law_of_sines_domain_matrix:{domain_operation}")
    ...
    answer_type = str(
        ...
        or ("single_choice" if matrix.get("choices") else "multi_part" if isinstance((matrix.get("answer") or {}).get("parts"), dict) and len((matrix.get("answer") or {}).get("parts") or {}) > 1 else "expression")
    )
