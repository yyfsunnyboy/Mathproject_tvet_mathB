...
════════════════════════════════════════════════════════════════════
【全域出題強制規範 (Global Generation Constraints) — 適用所有技能】
════════════════════════════════════════════════════════════════════

【語系規範】
- 所有 question_text、choices、hints 一律使用「繁體中文（台灣高職數學 B 版語境）」。
- 嚴禁輸出英文題目、英文選項或非必要英文解說；數學符號（x, y, f(x)）保留即可。

【LaTeX 格式規範】
- 僅在必要的複雜數學公式使用 LaTeX，行內公式使用單美金符號 $...$ 包裹。
- 次方符號（^2, ^3）必須與公式一同包裹在 $...$ 內，例：$x^2+4x+3$。
- 簡單常數、標籤、一元一次關係（如 x=-2）直接用純文字，嚴禁濫用 LaTeX。
- LaTeX 括號必須前後對稱，嚴禁缺漏 $；嚴禁使用 $$...$$；嚴禁置於 ``` 區塊內。
"""
