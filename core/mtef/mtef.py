...
class MTEF:
    ...
    def readRecord(self):
        ...
        while True:
            ...
            # Future records (>=100): skip payload length byte + body.
            if record >= RecordType.FUTURE:
                skip_len_bytes = self.reader.read(1)
                if not skip_len_bytes:
                    break
                skip_future_length = Helper.bytes2int(skip_len_bytes) or 0
                self.reader.seek(skip_future_length, 1)
                continue
    ...
    def makeLatex(self, ast):
        ...
        if ast.tag == RecordType.ROOT:
            ...
            char = chr(mtcode) if isinstance(mtcode, int) and mtcode >= 0 else ""

            # 生成char的一些特殊集
            hexExtend = ""
            typefaceFmt = ""
            face = (typeface - 128) if isinstance(typeface, int) else -1
            if face in {
                CharTypeface.fnMTEXTRA,
                CharTypeface.fnSPACE,
                CharTypeface.fnSYMBOL,
                CharTypeface.fnLCGREEK,
                CharTypeface.fnUCGREEK,
                CharTypeface.fnVECTOR,
            }:
                hexExtend = "/mathmode"
            elif face == CharTypeface.fnTEXT:
                typefaceFmt = "{ \\rm{ %s } }"
